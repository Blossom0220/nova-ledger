package com.nova.ledger.repository;

import com.nova.ledger.entity.Category;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface CategoryRepository extends JpaRepository<Category, Long> {

    List<Category> findByBookIdAndDeletedFalseOrderBySortOrderAsc(Long bookId);

    List<Category> findByBookIdAndTypeAndDeletedFalseOrderBySortOrderAsc(Long bookId, Category.CategoryType type);

    List<Category> findByParentIdAndDeletedFalse(Long parentId);

    boolean existsByParentIdAndDeletedFalse(Long parentId);
}
